...
public class ReplaceManifest
{
    ...
    public ReplaceManifest(Long node, Identifier ark, File currentManifest, File builtManifest, LoggerInf logger)
        throws TException
    {
        this.node = node;
        this.ark = ark;
    ...
    public ReplaceManifest(NodeIO.AccessNode nodeAccess, Identifier ark, File currentManifest, File builtManifest, LoggerInf logger)
        throws TException
    {
        this.node = node;
        this.ark = ark;
    ...
    {
        String path = "/home/loy/MRTMaven/github/admin/mrt-store-admin/tasks/210420-current/fix/manifests/R30C70";
        File source = new File(path);
        File currentMan = new File(source, "current.xml");
        File builtMan = new File(source, "built.xml");
        long node = 4001L;
        Identifier ark = new Identifier("ark:/90135/q1dv1gt9");
        
        LoggerInf logger = new TFileLogger(NAME, 50, 50);
        ReplaceManifest fixRM = new ReplaceManifest(node, ark, currentMan, builtMan, logger);
        fixRM.test();
    }
    
    public static void main_prod(String[] args) 
            throws IOException,TException 
    {
        String path = "/home/loy/MRTMaven/github/admin/mrt-store-admin/tasks/210420-current/fix/manifests/R30C70";
        File source = new File(path);
        ...
        LoggerInf logger = new TFileLogger(NAME, 50, 50);
        ReplaceManifest fixDD = new ReplaceManifest(node, ark, currentMan, builtMan, logger);
        fixDD.replace();
        ...
        throws TException
    {
        ...
        String fromID = fromMap.getObjectID().getValue();
        String toID = toMap.getObjectID().getValue();
        if (debug)System.out.println( "match:"
                + " - fromID:" + fromID + "\n"
                + " - toID:" + toID + "\n"
                + " - fromCur:" + fromMap.getCurrent() + "\n"
                + " - toCur:" + toMap.getCurrent() + "\n"
        );
    ...
    protected boolean isBuiltManifest()
        throws TException
    {
        try {
            if ((existingManifest == null) || !existingManifest.exists()) {
    ...
    public VersionMap getVersionMap(File mapFile)
        throws TException
    {
        return VersionMapTools.getFile(mapFile, logger);
    }
